...
class Acl_Table(Executor):
    ...
    def get_all_args(self, ns=""):
        ...
        acl_tables = ret["keys"]
        return [key.split("|")[-1] for key in acl_tables]
    ...
    def init_acl_table_config_info(self, acl_table_name):
        ...
        acl_table_type_name = ret["return_values"].get(f"ACL_TABLE|{acl_table_name}", {}).get("type")
    ...
    def init_acl_table_asic_info(self, acl_table_name):
        req = MatchRequest(db="CONFIG_DB", table="ACL_RULE", key_pattern=f"{acl_table_name}|*", ns=self.ns)
        ret = self.match_engine.fetch(req)
        acl_rules = ret["keys"]
        if not acl_rules:
            return
        acl_rule_name = acl_rules[0].split('|')[-1]

        counter_oid = fetch_acl_counter_oid(self.match_engine, acl_table_name, acl_rule_name, self.ns)
        if not counter_oid:
            return

        req = MatchRequest(db="ASIC_DB", table="ASIC_STATE:SAI_OBJECT_TYPE_ACL_COUNTER", key_pattern=counter_oid,
                           return_fields=["SAI_ACL_COUNTER_ATTR_TABLE_ID"], ns=self.ns)
        ret = self.match_engine.fetch(req)

        table_oid = ret["return_values"].get(f"ASIC_STATE:SAI_OBJECT_TYPE_ACL_COUNTER:{counter_oid}", {}).get("SAI_ACL_COUNTER_ATTR_TABLE_ID")
        if not table_oid:
            return

        req = MatchRequest(db="ASIC_DB", table="ASIC_STATE:SAI_OBJECT_TYPE_ACL_TABLE", key_pattern=table_oid, ns=self.ns)
        ret = self.match_engine.fetch(req)
        self.add_to_ret_template(req.table, req.db, ret["keys"], ret["error"])

        req = MatchRequest(db="ASIC_DB", table="ASIC_STATE:SAI_OBJECT_TYPE_ACL_TABLE_GROUP_MEMBER", key_pattern="*",
                           field="SAI_ACL_TABLE_GROUP_MEMBER_ATTR_ACL_TABLE_ID", value=table_oid, return_fields=["SAI_ACL_TABLE_GROUP_MEMBER_ATTR_ACL_TABLE_GROUP_ID"], ns=self.ns)
        ret = self.match_engine.fetch(req)
        self.add_to_ret_template(req.table, req.db, ret["keys"], ret["error"])
        ...
        for group_oid in group_oids:
            req = MatchRequest(db="ASIC_DB", table="ASIC_STATE:SAI_OBJECT_TYPE_ACL_TABLE_GROUP", key_pattern=group_oid, ns=self.ns)
            ret = self.match_engine.fetch(req)
            self.add_to_ret_template(req.table, req.db, ret["keys"], ret["error"])
