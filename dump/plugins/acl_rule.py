...
class Acl_Rule(Executor):
    ...
    def get_all_args(self, ns=""):
        ...
        acl_rules = ret["keys"]
        return [key.split("|", 1)[-1] for key in acl_rules]

    def execute(self, params):
        self.ret_temp = create_template_dict(dbs=["CONFIG_DB", "ASIC_DB"])

        try:
            acl_table_name, acl_rule_name = params[self.ARG_NAME].split('|', 1)
        except ValueError:
    ...
    def init_acl_rule_config_info(self, acl_table_name, acl_rule_name):
        req = MatchRequest(db="CONFIG_DB", table="ACL_RULE", key_pattern=f"{acl_table_name}|{acl_rule_name}", ns=self.ns)
        ret = self.match_engine.fetch(req)
    ...
    def init_acl_rule_asic_info(self, acl_table_name, acl_rule_name):
        ...
        req = MatchRequest(db="ASIC_DB", table="ASIC_STATE:SAI_OBJECT_TYPE_ACL_ENTRY", key_pattern="*",
                           field="SAI_ACL_ENTRY_ATTR_TABLE_ID", value=table_oid, return_fields=["SAI_ACL_ENTRY_ATTR_FIELD_ACL_RANGE_TYPE"], ns=self.ns)
        ret = self.match_engine.fetch(req)
        self.add_to_ret_template(req.table, req.db, ret["keys"], ret["error"])
        ...
        for _, entry in ret["return_values"].items():
            ...
            if not range_attr_value:
                continue
            ranges_attr_value = range_attr_value.split(':', 1)
        ...
        for range_oid in range_oids:
            req = MatchRequest(db="ASIC_DB", table="ASIC_STATE:SAI_OBJECT_TYPE_ACL_RANGE", key_pattern=range_oid, ns=self.ns)
            ret = self.match_engine.fetch(req)
            self.add_to_ret_template(req.table, req.db, ret["keys"], ret["error"])
