    ...
    def encode_data(self, json_data):
        ...
        for key, value in json_data.items():
            ...
            for pb, bin_data in value.items():
                bin_pb = pb.encode()
                bin_data_list = bin_data.split("_")
                bin_int_list = [int(x) for x in bin_data_list]
                str1= ''.join(chr(values) for values in bin_int_list)
                new_value[bin_pb] = str1.encode('latin-1')
