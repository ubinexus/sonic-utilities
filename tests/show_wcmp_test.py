...
class TestWcmpMultipleAsic:
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                {"asic0": os.path.join(mock_config_path, "disabled"),
                 "asic1": os.path.join(mock_config_path, "disabled")},
                {"plain": assert_show_output.show_wcmp_status_multi_disabled,
                 "json": assert_show_output.show_wcmp_status_multi_disabled_json},
                id="all-disabled"
            ),
            pytest.param(
                {"asic0": os.path.join(mock_config_path, "enabled"),
                 "asic1": os.path.join(mock_config_path, "disabled")},
            ...
            pytest.param(
                {"asic0": os.path.join(mock_config_path, "disabled"),
                 "asic1": os.path.join(mock_config_path, "enabled")},
    ...
    @patch("show.wcmp_cli.multi_asic.is_multi_asic", mock.Mock(return_value=True))
    @patch("show.wcmp_cli.multi_asic.get_namespace_list", mock.Mock(return_value=["asic0", "asic1"]))
    def test_show_wcmp_status_multi_asic(self, cfgdb, output, format):
        dbconnector.namespaces = {
            "asic0": {
                "CONFIG_DB": cfgdb["asic0"]
            },
            "asic1": {
                "CONFIG_DB": cfgdb["asic1"]
            }
        }
            
        runner = CliRunner()
        cmd_args = ["--json"] if format == "json" else []
        result = runner.invoke(show.cli.commands["w-ecmp"].commands["status"], cmd_args)
