...
class TestReloadConfig(object):
    dummy_cfg_file = os.path.join(os.sep, "tmp", "config.json")
    dummy_cfg_file_contents = os.path.join(mock_db_path, "config_db.json")#os.path.join(os.sep, "tmp", "config.json")
...
class TestConfigLoopback(object):
    ...
    @patch("sonic_py_common.device_info.is_yang_config_validation_enabled", mock.Mock(return_value=True))
