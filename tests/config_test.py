...
class TestConfigLoopback(object):
    ...
    def test_add_loopback_with_invalid_name_adhoc_validation(self):
        ...
        print(result.output)
        assert result.exit_code != 0
        assert "Error: Loopbax1 is invalid, name should have prefix 'Loopback' and suffix '<0-999>', such that 'loopback_name' not cross 11 characters" in result.output

        result = runner.invoke(config.config.commands["loopback"].commands["add"], ["Loopback0000"], obj=obj)
        print(result.exit_code)
        print(result.output)
        assert result.exit_code != 0
        assert "Error: Loopback0000 is invalid, name should have prefix 'Loopback' and suffix '<0-999>', such that 'loopback_name' not cross 11 characters" in result.output
