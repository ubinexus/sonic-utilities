...
def _get_sonic_services():
    cmd0 = ['systemctl', 'list-dependencies', '--plain', 'sonic.target']
    cmd1 = ['sed', '1d']
    out, _ = clicommon.run_command_pipe(cmd0, cmd1, return_cmd=True)
    return (unit.strip() for unit in out.splitlines())


def _get_delayed_sonic_units(get_timers=False):
    cmd0 = ['systemctl', 'list-dependencies', '--plain', 'sonic-delayed.target']
    cmd1 = ['sed', '1d']
    rc1, _ = clicommon.run_command_pipe(cmd0, cmd1, return_cmd=True)
...
def _delay_timers_elapsed():
    for timer in _get_delayed_sonic_units(get_timers=True):
        out, _ = clicommon.run_command(['systemctl', 'show', str(timer), '--property=LastTriggerUSecMonotonic', '--value'], return_cmd=True)
        if out.strip() == "0":
            return False
    return True
...
def load_backend_acl(cfg_db, device_type):
    ...
    if device_type and device_type == "BackEndToRRouter" and _is_storage_device(cfg_db) and _is_acl_table_present(cfg_db, "DATAACL"):
        if os.path.isfile(BACKEND_ACL_TEMPLATE_FILE):
            clicommon.run_command(
                [SONIC_CFGGEN_PATH, '-d', '-t', '{},{}'.format(BACKEND_ACL_TEMPLATE_FILE, BACKEND_ACL_FILE)],
                display_cmd=True
            )
        if os.path.isfile(BACKEND_ACL_FILE):
            clicommon.run_command(['acl-loader', 'update', 'incremental', str(BACKEND_ACL_FILE)], display_cmd=True)
...
def load_mgmt_config(filename):
    ...
    if len(config_data['MGMT_INTERFACE'].keys()) > 0:
        ...
        cmd1 = ['kill', str(pid)]
        cmd2 = ['rm', '-f', '/var/run/dhclient.eth0.pid']
        clicommon.run_command_AND(cmd0, cmd1, cmd2, display_cmd=True, ignore_error=True)
