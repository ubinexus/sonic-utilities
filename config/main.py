...
def dhcp_relay_intf_type_validate(interface_name):
    interface_type = None
...
def dhcp_relay_intf_validate(ctx, interface_name, interface_type):
...
#
# 'add' command ('config interface ip dhcp-relay add ...')
#
@dhcp_relay.command('add')
@click.argument('interface_name', metavar='<interface_name>', required=True)
@click.argument('ip_addr1', metavar='<ip_addr1>', required=True)
...
def dhcp_relay_add(ctx, interface_name, ip_addr1, ip_addr2, ip_addr3, ip_addr4, src_intf, link_select):
    ...
    if not is_ip4_addr_valid(ip_addr1, True):
        ctx.fail("IP address {} is not valid".format(ip_addr1))
    if ip_addr2:
        if not is_ip4_addr_valid(ip_addr2, True):
            ctx.fail("IP address {} is not valid".format(ip_addr2))
    if ip_addr3:
        if not is_ip4_addr_valid(ip_addr3, True):
            ctx.fail("IP address {} is not valid".format(ip_addr3))
    if ip_addr4:
        if not is_ip4_addr_valid(ip_addr4, True):
            ctx.fail("IP address {} is not valid".format(ip_addr4))

    dhcp_servers = intf.get('dhcp_servers', [])

    if len(dhcp_servers) is 4:
        ctx.fail("Maximum number of Servers configured on the relay interface {}".format(interface_name))

    ignore_addrs = []

    if ip_addr1 in dhcp_servers:
        ctx.fail("IP address {} is already configured.".format(ip_addr1))
    else:
        dhcp_servers.append(ip_addr1)
        addrs = str(ip_addr1)

    if ip_addr2:
        if ip_addr2 in dhcp_servers:
            ctx.fail("IP address {} is already configured.".format(ip_addr2))
        else:
            if len(dhcp_servers) is 4:
                ignore_addrs.append(str(ip_addr2))
            else:
                dhcp_servers.append(ip_addr2)
                addrs += "," + str(ip_addr2)

    if ip_addr3:
        if ip_addr3 in dhcp_servers:
            ctx.fail("IP address {} is already configured.".format(ip_addr3))
        else:
            if len(dhcp_servers) is 4:
                ignore_addrs.append(str(ip_addr3))
            else:
                dhcp_servers.append(ip_addr3)
                addrs += "," + str(ip_addr3)

    if ip_addr4:
        if ip_addr4 in dhcp_servers:
            ctx.fail("IP address {} is already configured.".format(ip_addr4))
        else:
            if len(dhcp_servers) is 4:
                ignore_addrs.append(str(ip_addr4))
            else:
    ...
    click.echo("Added DHCP relay address {} to {}".format(addrs, interface_name))
    if len(ignore_addrs) != 0:
        click.echo("Maximum number of servers configured. Ignored adresses {}".format(ignore_addrs))
...
#
# 'remove' command ('config interface ip dhcp-relay remove ...')
#
@dhcp_relay.command('remove')
@click.argument('interface_name', metavar='<interface_name>', required=True)
@click.argument('ip_addr1', metavar='<ip_addr1>', required=True)
@click.argument('ip_addr2', metavar='<ip_addr2>', required=False)
@click.argument('ip_addr3', metavar='<ip_addr3>', required=False)
...
def dhcp_relay_remove(ctx, interface_name, ip_addr1, ip_addr2, ip_addr3, ip_addr4):
    ...
    if intf is None:
        ctx.fail("'interface_name' not found. Please check if interface exists and an IP address is assigned")
    ...
    if (not ip_addr1 in dhcp_servers):
        ctx.fail("DHCP Relay IP address {} is not configured.".format(ip_addr1))
    else:
        dhcp_servers.remove(ip_addr1)
        addrs = str(ip_addr1)

    if ip_addr2:
        if (not ip_addr2 in dhcp_servers):
            ctx.fail("DHCP Relay IP address {} is not configured.".format(ip_addr2))
        else:
           dhcp_servers.remove(ip_addr2)
           addrs += "," + str(ip_addr2)

    if ip_addr3:
        if (not ip_addr3 in dhcp_servers):
            ctx.fail("DHCP Relay IP address {} is not configured.".format(ip_addr3))
        else:
            dhcp_servers.remove(ip_addr3)
            addrs += "," + str(ip_addr3)

    if ip_addr4:
        if (not ip_addr4 in dhcp_servers):
            ctx.fail("DHCP Relay IP address {} is not configured.".format(ip_addr4))
        else:
            dhcp_servers.remove(ip_addr4)
            addrs += "," + str(ip_addr4)
