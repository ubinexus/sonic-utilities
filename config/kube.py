...
def _update_kube_server(field, val):
    ...
    key = "SERVER"
    db_data = _get_db_data(table, key)
    def_data = {
        "IP": "",
        "insecure": "False",
        "disable": "False"
...
def _download_file(server, insecure):
    ...
    if r.status_code == 200:
        ...
        _do_exit("Failed to download {}".format(SERVER_ADMIN_URL.format(server)))   
...
def _get_labels():
    ...
    lh = _get_db_data('DEVICE_METADATA', 'localhost')
...
def _do_join(server,insecure):
    ...
    try:
        ...
        run_command(KUBEADM_JOIN_CMD.format(hostname()), ignore_error=True)
