...
class UbootBootloader(OnieInstallerBootloader):
    ...
    def get_installed_images(self):
        images = []
        proc = subprocess.Popen("/usr/bin/fw_printenv -n sonic_version_1", shell=True, universal_newlines=True, stdout=subprocess.PIPE)
        (out, _) = proc.communicate()
        image = out.rstrip()
        if IMAGE_PREFIX in image:
            images.append(image)
        proc = subprocess.Popen("/usr/bin/fw_printenv -n sonic_version_2", shell=True, universal_newlines=True, stdout=subprocess.PIPE)
        (out, _) = proc.communicate()
        image = out.rstrip()
        if IMAGE_PREFIX in image:
            images.append(image)
    ...
    def get_next_image(self):
        ...
        proc = subprocess.Popen("/usr/bin/fw_printenv -n boot_next", shell=True, universal_newlines=True, stdout=subprocess.PIPE)
        (out, _) = proc.communicate()
        image = out.rstrip()
